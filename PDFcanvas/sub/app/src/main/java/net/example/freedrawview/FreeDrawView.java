...
public class FreeDrawView extends View implements View.OnTouchListener {
    ...
    public FreeDrawView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        currentState = DrawState.FreeDraw;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mPaths.size(); i++) {
            ...
            if (currentPath.isPoint()) {
            ...
            } else if (currentPath.isCircle) {
                float x1 = currentPath.getOriginX();
                float y1 = currentPath.getOriginY();
                float x2 = currentPath.finalX;
                float y2 = currentPath.finalY;
                if (x1 > x2) {
                    float temp = x1;
                    x1 = x2;
                    x2 = temp;
                ...
                canvas.drawOval(x1,y1,x2,y2,currentPath.getPaint());
            } else {// Else draw the complete path
                if (currentPath.getPath().actions.size() == 2) {
                    canvas.drawLine(currentPath.getOriginX(), currentPath.getOriginY(), currentPath.finalX, currentPath.finalY, currentPath.getPaint());
                } else
        ...
        if (mPoints.size() == 1 || FreeDrawHelper.isAPoint(mPoints)) {
            ...
            if (currentState == DrawState.CircleDraw) {
                float x1 = mPoints.get(0).x;
                float y1 = mPoints.get(0).y;
                float x2 = mPoints.get(mPoints.size() - 1).x;
                float y2 = mPoints.get(mPoints.size() - 1).y;
                ;
                if (x1 > x2) {
                    float temp = x1;
                    x1 = x2;
                ...
                canvas.drawOval(x1, y1, x2, y2, mCurrentPaint);
            } else if (currentState == DrawState.LineDraw && mPoints.size() == 2) {
                canvas.drawLine(mPoints.get(0).x,mPoints.get(0).y,mPoints.get(1).x,mPoints.get(1).y,mCurrentPaint);
            } else {
                boolean first = true;
    ...
    private void createHistoryPathFromPoints() {
        if (currentState == DrawState.LineDraw) {
            SerializablePath linePath = new SerializablePath();
            linePath.moveTo(mPoints.get(0).x, mPoints.get(0).y);
            if (mPoints.size() > 1)
                linePath.lineTo(mPoints.get(mPoints.size() - 1).x, mPoints.get(mPoints.size() - 1).y);
            mPaths.add(new HistoryPath(
                    new SerializablePath(linePath), new SerializablePaint(mCurrentPaint),
                    mPoints.get(0).x, mPoints.get(0).y, mPoints.get(mPoints.size() - 1).x, mPoints.get(mPoints.size() - 1).y,
                    FreeDrawHelper.isAPoint(mPoints), false));
        } else
            mPaths.add(new HistoryPath(
                    new SerializablePath(mCurrentPath), new SerializablePaint(mCurrentPaint),
                    mPoints.get(0).x, mPoints.get(0).y, mPoints.get(mPoints.size() - 1).x, mPoints.get(mPoints.size() - 1).y,
                    FreeDrawHelper.isAPoint(mPoints), currentState == DrawState.CircleDraw));
    ...
    public ArrayList<HistoryPath> getPathsClone() {
        ArrayList<HistoryPath> newPath = new ArrayList<HistoryPath>();
        for (HistoryPath path : mPaths)
            newPath.add(path);
        return newPath;
    }

    public void restorePaths(ArrayList<HistoryPath> paths) {
        mPaths.clear();
        if (paths != null)
            for (HistoryPath path : paths)
                mPaths.add(path);
    }
